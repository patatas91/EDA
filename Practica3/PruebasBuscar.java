...
public class PruebasBuscar<Id extends Comparable<Id>, In> {
	...
	private static ColeccionTrie<Informacion> trie = new ColeccionTrie<Informacion>();
	...
	private static void informacionAleatoria(int n,
			...
			ColeccionTrie<Informacion> trie) {
		// Genera n usuarios
		for (int i = 0; i < n; i++) {
			// Genera numero cuenta corriente aleatorio
	...
	/**
	 * Genera n busquedas sobre la estructura ColeccionTrie con el identificador
	 * <identificador>
	 * 
	 * @param n
	 * @param identificador
	 */
	private static void buscarTrie(int n, String identificador) {
		for (int i = 0; i < n; i++) {
			trie.esta(identificador);
		}
	}
	...
	private static void vaciarEstructuras() {
		...
		trie = new ColeccionTrie<Informacion>();
	}
	...
	public static void main(String[] args) {
		...
		informacionAleatoria(1000, arbol, trie);
		...
		System.out
				.println("   Tiempo para class tads.colecciones.ColeccionABB: "
						+ totalTiempo + " ms.");
		// Calcula el Tiempo -> TRIE
		tiempoInicio = System.currentTimeMillis();
		buscarTrie(10000, numCuentaAleatorio());
		totalTiempo = System.currentTimeMillis() - tiempoInicio;
		System.out
				.println("   Tiempo para class tads.colecciones.ColeccionTrie: "
						+ totalTiempo + " ms.\n");
		...
		informacionAleatoria(10000, arbol, trie);
		...
		informacionAleatoria(100000, arbol, trie);
