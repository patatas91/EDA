...
public class ColeccionAbb<Id extends Comparable<Id>, In> implements
	...
	private Nodo<Par<Id, In>> meterRec(Par<Id, In> elemento,
		...
		else if (elemento.getPrimero()
				.compareTo(aux.getElemento().getPrimero()) < 0) {
	...
	private In obtenerInformacionRec(Nodo<Par<Id, In>> aux, Id identificador)
		...
		if (aux != null) {
			In info = null;
			// si la clave es menor -> IZQUIERDA
			if (identificador.compareTo(aux.getElemento().getPrimero()) < 0) {
				return obtenerInformacionRec(aux.getIzquierda(), identificador);
			}
			// si la clave es mayor -> DERECHA
			else if (identificador.compareTo(aux.getElemento().getPrimero()) > 0) {
				return obtenerInformacionRec(aux.getDerecha(), identificador);
			}
			// cuando lo encuentra
			else {
				info = aux.getElemento().getSegundo();
				return info;
			}
		} else
	...
	private Nodo<Par<Id, In>> cambiarMax(Nodo<Par<Id, In>> aux2)
		...
		Id identificador = (actual.getElemento().getPrimero());
		In informacion = (actual.getElemento().getSegundo());
		Par<Id, In> elemento = new Par<Id, In>(identificador, informacion);
		// Lo borra
		aux2 = borrarRec(actual.getElemento().getPrimero(), aux2);
	...
	public Par<Id, In> siguiente() throws NoHaySiguienteException,
		...
		if (haySiguiente()) {
			...
			Par<Id, In> sig = new Par<Id, In>(aux.getElemento().getPrimero(),
					aux.getElemento().getSegundo());
			aux = aux.getDerecha();
			while (aux != null) {
				pilaArbol.apilar(aux);
				aux = aux.getIzquierda();
