...
public class ColeccionLista<Id extends Comparable<Id>, In> implements
	...
	private Celda<Par<Id, In>> primera;
	private Celda<Par<Id, In>> actual;
	private int total;
	...
	public boolean meter(Id identificador, In informacion) {
		...
		Par<Id, In> par = new Par<Id, In>(identificador, informacion);
		Celda<Par<Id, In>> aux = new Celda<Par<Id, In>>(par);
		// si esta vacia
		if (esVacia()) {
			primera = aux;
			total = 1;
			...
			if (identificador.compareTo(primera.getElemento()
					.getIdentificador()) < 0) {
			...
			// busca punto de insercion
				...
				Celda<Par<Id, In>> anterior;
				anterior = primera;
				actual = primera;
				// itera mientras la clave sea menor a la clave
				// del elemento
				while (actual.getSiguiente() != null
						&& (actual.getSiguiente().getElemento()
								.getIdentificador()).compareTo(identificador) <= 0) {
					anterior = actual;
					actual = actual.getSiguiente();
				}
				// la clave es igual a la clave del elemento, cambia el valor
				if (identificador.compareTo(actual.getElemento()
						.getIdentificador()) == 0) {
					Par<Id, In> p = new Par<Id, In>(actual.getElemento()
							.getIdentificador(), informacion);
					actual.setElemento(p);
				}
	...
	public boolean borrar(Id identificador) {
		...
		// Celdas auxiliares
		Celda<Par<Id, In>> aux;
		Celda<Par<Id, In>> aux2;
		// comprueba que tiene elementos
		if (!esVacia()) {
			// si esta el primero
			if (primera.getElemento().getIdentificador()
