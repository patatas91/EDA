...
public class ColeccionAbb<Id extends Comparable<Id>, In> implements
	...
	private Nodo<Par<Id, In>> meterRec(Par<Id, In> elemento,
		...
		else if (elemento.getIdentificador().compareTo(
				aux.getElemento().getIdentificador()) < 0) {
	...
	private In obtenerInformacionRec(Nodo<Par<Id, In>> aux, Id identificador)
		...
		if (aux != null) {
			...
			// si la clave es mayor -> DERECHA
			else if (identificador.compareTo(aux.getElemento()
					.getIdentificador()) > 0) {
	...
	public Par<Id, In> siguiente() throws NoHaySiguienteException {
		try {
			if (haySiguiente()) {
				...
				Par<Id, In> sig = new Par<Id, In>(aux.getElemento()
						.getIdentificador(), aux.getElemento().getInformacion());
				aux = aux.getDerecha();
				while (aux != null) {
					pilaArbol.apilar(aux);
					aux = aux.getIzquierda();
